...
public class OperatorInput extends SubsystemBase {
    ...
    public double getCBSpeed() {
        shooterController.setAxisDeadband(0.05);
    ...
    public boolean getLongRangeShooterSpeed() {
        return shooterController.getXButton(); // If the DPad is pressed down
    }

    public boolean getMediumRangeShooterSpeed() {
        return shooterController.getYButton(); // If the DPad is pressed to the right
    }

    public boolean getShortRangeShooterSpeed() {
        return shooterController.getBButton(); // If the DPad is pressed up. DPad not being pressed will return -1, not 0.
    }

    public boolean getShooterMode() {
        return shooterController.getRightTriggerAxis() > 0.1 || shooterController.getAButton(); // Does not handle logic for the
                                                                                                // switch.
    }
}
